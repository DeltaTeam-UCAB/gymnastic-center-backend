...
import { test, beforeAll, beforeEach, cleanup } from 'assert-raisins'
...
export const raisinsTestSuitDeclartion: SuitDeclaration = async (
    name: string,
    data = {},
) => {
    ...
    afterEachs.map((e) => beforeEach(() => cleanup(e)))
    afterAlls.map((e) => beforeAll(() => cleanup(e)))
    beforeEachs.map((e) => beforeEach(e))
    beforeAlls.map((e) => beforeAll(e))
