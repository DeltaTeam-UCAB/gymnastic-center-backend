...
import { DeleteBlogsByTrainerResponse } from './types/response'
...
export class DeleteBlogsByTrainerCommand
    implements
        ApplicationService<
            DeleteBlogsByTrainerDTO,
            DeleteBlogsByTrainerResponse
        >
{
    constructor(
    ...
    async execute(
        data: DeleteBlogsByTrainerDTO,
    ): Promise<Result<DeleteBlogsByTrainerResponse>> {
        const blogs = await this.blogRepository.getAllByTrainer(
            new TrainerId(data.id),
        )
        blogs.forEach((b) => {
            b.delete()
            this.blogRepository.delete(b)
            this.eventPublisher.publish(b.pullEvents())
        })
        return Result.success({ blogsDeleted: blogs.length })
    }
}
