...
export class CreatePostController implements ControllerContract<[body: CreatePostDTO] , {id: string}> {
    ...
    async execute(@Body() body: CreatePostDTO): Promise<{id: string}> {
        ...
        const existingPost = await this.postRepo.findOneBy({ title: postDetails.title });
        if (existingPost) {
            throw new HttpException('A post with this title already exists', 400);
        }

        const postId = this.idGen.generate();
        const post = this.postRepo.create(postDetails);
        await this.postRepo.save([{id: postId, ...postDetails}]);

        for (const image of images) {
            const imageId = this.idGen.generate();
            const imageEntity = this.imagePostRepo.create({id: imageId, url: image, postId });
